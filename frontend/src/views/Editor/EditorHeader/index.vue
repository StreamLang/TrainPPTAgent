  ...
  <div class="editor-header">
    ...
    <div class="right">
        ...
        <div class="menu-item" v-tooltip="'插入base64图片'" @click="createImgElementBase64()">
          <IconPicture class="icon" /> 插入图片(base64)
        </div>
        <div class="menu-item" v-tooltip="'插入饼状图'" @click="createPieChart()">
          <IconChartProportion class="icon" /> 插入饼状图
        </div>
        <div class="menu-item" v-tooltip="'插入柱状图'" @click="createBarChart()">
          <IconHamburgerButton class="icon" /> 插入柱状图
        </div>
        <div class="menu-item" v-tooltip="'插入折线图'" @click="createLineChart()">
          <IconConnection class="icon" /> 插入折线图
        </div>
        <div class="menu-item" v-tooltip="'插入数据表格'" @click="importTableData()">
          <IconPicture class="icon" /> 插入表格数据(CSV)
        </div>
...
const createPieChart = () => {
  ...
  // 对于饼图，labels 代表各个扇区（即各个区域）的名称
    labels: ['华东地区', '华北地区', '华中地区', '华南地区', '西北地区', '东北地区', '西南地区'],
    // 饼图通常只有一个数据系列，所以 legends 数组只有一个元素
    legends: ['2025年1-8月累计用电量 (亿千瓦时)'],
    // series 是一个二维数组，这里只包含一个子数组，每个数值对应一个区域的用电量
    series: [
      [3200, 1800, 2400, 2100, 1600, 1300, 1900]
    ]
  })
}
...
const createBarChart = () => {
  createChartElement('bar', {
    ...
    legends: ['家庭用电', '工业用电'],
    series: [
      [300, 280, 350, 400, 450, 500, 600, 700, 650, 550, 450, 350], // 家庭用电
      [1500, 1400, 1600, 1700, 1800, 1900, 2000, 2100, 2000, 1900, 1700, 1600] // 工业用电
    ]
  })
}
